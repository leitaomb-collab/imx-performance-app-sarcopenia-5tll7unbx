...
function formatEvolution(
  ...
  if (prev === '-' || prev === '') return '-'
  if (!delta || delta.text === '-') return `${prev} =`
  const match = delta.text.match(/[+-]\d+\.?\d*/)
  return match ? `${prev} ${match[0]}` : `${prev} =`
}
    ...
    <>
      ...
      <div className="hidden md:grid md:grid-cols-1 lg:grid-cols-2 gap-3 print:grid print:grid-cols-1 summary-cards-grid">
        {cards.map((card, index) => (
          <div
            key={`dt-${card.title}`}
            className="summary-card animate-summary-card rounded-lg border p-3 shadow-sm break-inside-avoid"
              ...
              <table className="summary-table w-full">
                <thead>
                  <tr className="text-muted-foreground border-b border-border">
                    <th
                      scope="col"
                      className="text-left py-1 pr-2 font-medium text-xs lg:text-[0.5625rem] max-w-32 truncate lg:max-w-none lg:overflow-visible"
                    >
                      Parâmetro
                    </th>
                    <th
                      scope="col"
                      className="text-right py-1 px-2 lg:px-1 font-medium text-xs lg:text-[0.5625rem]"
                    >
                      Atual
                    </th>
                    <th
                      scope="col"
                      className="text-right py-1 px-2 lg:px-1 font-medium text-xs lg:hidden w-12 min-w-12"
                    >
                      Evol.
                    </th>
                    <th
                      scope="col"
                      className="text-right py-1 px-1 font-medium text-[0.5625rem] hidden lg:table-cell"
                    >
                      Ant.
                    </th>
                    <th
                    ...
                    <th
                      scope="col"
                      className="text-center py-1 px-2 lg:px-1 font-medium text-xs lg:text-[0.5625rem] w-20 min-w-20 lg:w-auto lg:min-w-0"
                    >
                      Status
                    </th>
                    <th
                      scope="col"
                      className="text-center py-1 pl-1 font-medium text-[0.5625rem] hidden lg:table-cell w-16 min-w-16"
                    >
                      Tend.
                    </th>
                  </tr>
                ...
                <tbody>
                  {card.rows.map((row) => (
                    <tr key={row.label} className="border-b border-border/30 last:border-0">
                      <th
                        scope="row"
                        className="text-left py-1 pr-2 font-medium text-xs lg:text-[0.6875rem] whitespace-nowrap max-w-32 truncate lg:max-w-none lg:overflow-visible"
                      >
                        {row.label}
                      </th>
                      <td className="text-right py-1 px-2 lg:px-1 tabular-nums text-sm font-bold lg:text-[0.6875rem] lg:font-normal">
                        {row.current}
                      </td>
                      <td className="text-right py-1 px-2 lg:px-1 tabular-nums text-xs text-muted-foreground w-12 min-w-12 lg:hidden">
                        {formatEvolution(row.previous, row.delta)}
                      </td>
                      <td className="text-right py-1 px-1 tabular-nums text-muted-foreground text-[0.6875rem] hidden lg:table-cell">
                        {row.previous}
                      </td>
                      <td
                        className={cn(
                          'text-right py-1 px-1 whitespace-nowrap text-[0.6875rem] hidden lg:table-cell',
                          deltaColor(row.delta?.direction),
                        )}
                      >
                        {row.delta?.text || '-'}
                      </td>
                      <td className="text-right py-1 px-2 lg:px-1 text-muted-foreground whitespace-normal text-xs">
                        {row.ref}
                      </td>
                      <td className="text-center py-1 px-2 lg:px-1 w-20 min-w-20 lg:w-auto lg:min-w-0">
                        {row.status ? <StatusBadge status={row.status} /> : '-'}
                      </td>
                      <td className="text-center py-1 pl-1 hidden lg:table-cell w-16 min-w-16">
                        <Sparkline
